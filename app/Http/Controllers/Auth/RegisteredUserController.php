    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->session()->get('invitation_token')) {
            $invitation = UserInvitation::where('token', $request->session()->get('invitation_token'))
                ...
                ->whereNull('registered_at')
                ->firstOr(fn () => throw ValidationException::withMessages(['invitation' => __('Invitation link does not match the email')]));
        ...
        if ($request->session()->get('activity') && $activity) {
            ...
            return redirect()->route('my-activity.show')->with('success', __('You have successfully registered.'));
        }
