            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 space-y-3">
                    @if(auth()->user()?->activities->contains($activity)) {{-- [tl! add:start] --}}
                        <div class="mb-6 bg-indigo-100 p-4 font-semibold text-indigo-700">{{ __('You have already registered.') }}</div>
                    @else
                        <form action="{{ route('activities.register', $activity) }}" method="POST">
                            @csrf

                            <x-secondary-button type="submit">
                                {{ __('Register to Activity') }}
                            </x-secondary-button>
                        </form>
                    @endif {{-- [tl! add:end] --}}
                    ...
                    <div>{{ __('Company') }}: {{ $activity->company->name }}</div>
